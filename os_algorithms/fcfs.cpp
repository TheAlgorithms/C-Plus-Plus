...
void find_completion_time(std::vector<process> &proc) {
	...
	unsigned end = 0;  ///< To maintain how many processes have finished execution
	...
	while(end < count) {
		...
		if (time < min_at)
			time = min_at;
		time += proc[idx].bt; ///< Add the burst time, to mark the end time of execution
		proc[idx].ct = time;  ///< Completion time is the end time of execution
		end++;
	}
}
 ...
 * @brief A function to calculate completion time and averages of turn-around and waiting time.
 *
 * @param proc A vector, which holds all processes, to be worked on.
 */
...
int main() {
	std::vector<process> proc = {
		...
		{4, 1, 2, 0},
		{5, 1, 1, 0}
	}; ///< A vector of process with corresponding data.
	...
	std::vector<process> test = {
		...
		{4, 1, 2, 7},
		{5, 1, 1, 8}
	}; ///< A vector to verify if the method executed correctly.
