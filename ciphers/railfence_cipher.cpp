...
namespace ciphers
{
    ...
    namespace railfence
    {
        ...
        std::string encrypt(const std::string &text, const int &key)
        {
            ...
            for (int i = 0; i < len; i++) // iterating through each character of provided string and putting it at the right place in rail matrix
            {
            ...
            for (int i = 0; i < key; i++)
            {
                for (int j = 0; j < len; j++) // iterating through each element of rail matrix left to right
                {
                    if (rail[i][j] != '-')
                    {
        ...
        std::string decrypt(const std::string &text, const int &key)
        {
            ...
            for (int i = 0; i < len; i++) // iterating through each character of provided string to mark the cells to be filled
            {
                if (r == 0 || r == key - 1) // if at first or last row
                    down = !down;           // reverse the direction of filling
