 ...
 namespace ciphers {
     ...
     */
 	namespace RailFence {   
		 ...
		 * @details
		 * Here key is the number of rail rows that will be used to encrpyt text. A 2-D character matrix is 
		 ...
		 */
 		...
 		std::string Cipher::rail_fence_encrypt(std::string text, int key){
		    ...
		    char rail[num_rows][num_columns];
		 ...
		 * @details
		 * The number of rails rows = key. The character matrix is created with key = rows, length(text) = columns 
		 * The rail matrix is filled to distinguish filled spaces from blank ones. A boolean is used to find the 
		 * direction for traversing. If the current row is either the first or last row, direction is reversed.
