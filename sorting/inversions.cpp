 ...
 * @brief [Inversion Count Algorithm](https://www.hackerrank.com/challenges/ctci-merge-sort/problem) 
 * 
 * @details
 ...
 * reverse order then inversion count is the maximum. Two elements a[i] and a[j]    
...
#include <cstdlib>  // C standard general utitlities library for operations like integer arithmetics, searching, sorting and converting etc
 ...
 */
int64_t merge(std::vector<int64_t> *arr, int64_t start, int64_t end)
{ 
    // mid point64_t of the array
    ...
    std ::vector<int64_t> temp(1000000);
 ...
 * @returns total number of inversions in the array and also sorts the array
 */
int64_t inversion_count(std::vector<int64_t> *arr, int64_t start, int64_t end)
{
    if (start >= end)
    {
        return 0;
...
static void test()
{
    std::vector<int64_t> arr = {9, 5, 3, 2};
...
int main()
{
    ...
    //input array
    std::vector<int64_t> arr = {9, 5, 3, 2};
    // should return and output 6 as the number of inversions are (9, 5), (5, 3), (9, 3), (9, 2), (5, 2), (3, 2)
    std::cout << inversion_count(&arr, 0, 3) << std::endl;
    return 0;
}
