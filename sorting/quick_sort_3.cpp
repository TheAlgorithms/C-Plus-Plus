...
/*This function takes first element as pivot and places  
  ...
  than pivot after it
*/
const std::vector<int> quick_sort_partition3(std::vector<int> &arr, int low, int high){
    ...
    std::vector<int> give_back(2);//this vector is used to return positions of higher and lower
    while (count <= q_e) {
        if (arr[count] < x) {
...
low to high to work as a pivot and then the at pivot with element at first position
*/
const void randomized_quick_sort(std::vector<int> &arr, int low, int high){
    ...
    std::vector<int> markers = quick_sort_partition3(arr, low, high);//Funtion quick_sort_partition3 is called which returns position
    randomized_quick_sort(arr, low, markers[0] - 1);                 //of elements lower than above value at random index and position
    randomized_quick_sort(arr, markers[1] + 1, high);                //of all elements greater than above value at random index and position
}
