#include <vector> // using for vector, push_back, begin, end, size, erase
#include <array> // using for array
#include <algorithm> // using for max_element, for_each, copy, reverse
#include <cmath> // using for pow
#include <string> // using for to_string
#include <iostream> // using for cout, cin, endl

/**
 * This function implements the Radix Sorting Algorithm based on the **less significant digit**
 * (LSD).
...
void radix(std::vector<int>& v)
{
    ...
    {
        // inter loop; insert each element into its respective bucket depending on which digit
        // position is being analyzed
        std::for_each(v.begin(), v.end(), [&buckets, i](int value)
            {
                buckets[(int)(value / std::pow(10, i)) % 10].push_back(value);
            });
...
/**
 * This function separates the main vector into two: one containing negative integers and one
