...
int main(){
     ...
     int 
          n,                  // the number of length of array
          lib_size,
          index_pos,
     ...
     cout<< "Enter the number of length of array : ";
     cin >> n;
     index =new int[n];
     ...
     while(index_pos<n){
          ...
          // binary search 
          int insert = distance(library[target_lib],
                                   lower_bound(library[target_lib],library[target_lib]+lib_size,index[index_pos]));

          // if there is no gap to insert a new index ...
          if(numbered[insert]==true){
               int
                    prov_size=0,
                    next_target_lib=!target_lib;
               ;
          ...
          // else... insert a new index to gap
          else{
     ...
     for(int i=0;i<n;i++){
          ...
          if(i<lib_size){
               cout<<library[target_lib][i]<<endl;
          }
     }
