 ...
 *
 * @details
 * Bubble sort algorithm is the bubble sorting algorithm. The most important
 * reason for calling the bubble is that the largest number is thrown at the end
 * of this algorithm. This is all about the logic. In each iteration, the
 * largest number is expired and when iterations are completed, the sorting
 * takes place.
 *
 * What is Swap?
 *
 ...
 * That's why we created a variable z to create the first value of the value of
 * x, and finally, we have assigned to y.
 *
 ...
 *
 * ### Worst Case
 * Bubble Sort Worst Case Performance is \f$O(n^{2})\f$. Why is that? Because if
 * you remember Big O Notation, we were calculating the complexity of the
 * algorithms in the nested loops. The \f$n * (n - 1)\f$ product gives us
 * \f$O(n^{2})\f$ performance. In the worst case all the steps of the cycle will
 * occur.
 *
 * ### Average Case
 * Bubble Sort is not an optimal algorithm. In average, \f$O(n^{2})\f$
...
static void test() {
    ...
    std::vector<int> sorted_4 = sorting::bubble_sort::bubble_sort(vec_4);

    std::vector<float> vec_5 = {1234, -273.1, 23, 150, 1234, 1555.55, -2000};
    std::vector<float> sorted_5 = sorting::bubble_sort::bubble_sort(vec_5);

    std::vector<char> vec_6 = {'z', 'Z', 'a', 'B', ' ', 'c', 'a'};
    std::vector<char> sorted_6 = sorting::bubble_sort::bubble_sort(vec_6);

    std::vector<std::string> vec_7 = {"Hello", "hello", "Helo", "Hi", "hehe"};
    std::vector<std::string> sorted_7 =
        sorting::bubble_sort::bubble_sort(vec_7);

    std::vector<std::pair<int, char>> vec_8 = {
        {10, 'c'}, {2, 'z'}, {10, 'a'}, {0, 'b'}, {-1, 'z'}};
    std::vector<std::pair<int, char>> sorted_8 =
        sorting::bubble_sort::bubble_sort(vec_8);
    ...
    assert(std::is_sorted(sorted_8.begin(), sorted_8.end()));
}
