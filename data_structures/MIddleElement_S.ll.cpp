...
/// Given that a particular linked list is sorted, limited to positive numbers, and not empty find median (New Change)
float median(node* head) {
    ...
    if (total % 2 == 0) {
        ...
        for (int i = 0; i < index; i++) {
            if (i == index - 1)
                return static_cast<float>(iter->data);
            iter = iter->next;
        }
    }
}
