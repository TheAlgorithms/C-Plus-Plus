 ...
 * @details Only implement some basic and complicated operations. Other operations should be easy to added.
...
bool Tree234::RemovePreMerge(Node *node, int item) {
	...
	while (node) {
		...
		// get here means both left sibling and right sibling of next_node is 2-node, so we do merge
...
void test1() {
    int arr[] = {3, 1, 5, 4, 2, 9, 10, 8, 7, 6, 16, 13, 14};
    Tree234 tree;
...
/** @brief simple test to insert continuous number of range [0, n), and print the tree*/
void test2(int n) {
    Tree234 tree;
