/**
 * @file
 * @brief An implementation of [graph](https://en.wikipedia.org/wiki/Graph_(abstract_data_type)) using
 ...
 *
 * The (u, v) pair of edges is ordered pair in case of a directed graph while this is not the case in undirected graphs.
 *
 * ##Implimentation:
 * In case of adjacency list, the index of vertices which are connected to a vertex is stored in a linked list connected to
 * that vertex. The structure looks like:
 *
 ...
 *      ...
 *
 * The main advantage of this implementation is that we are using only the space which is needed in other words
 * the space complexity is better than adjacency matrix where we use O(v^2) where v is the number of vertices.
 *
 * Unlike in case of adjacency matrix where we use space to store both info, connected and not connected, in case of
 * adjacency list we only store the info of is_connected.
 *
 * A disadvantage of this implementation over adjacency matrix is the time taken for finding an index in adjacency list,
 * because it'll be a linear search, time will be O(n), where in adjacency matrix it was constant as we has to access elements
 * by index.
 *
 * #Applications:
...
/**
 * @brief Allocates a new node in heap for given data as a node and returns it's pointer.
...
/**
 * @brief An insert function to create and add new nodes in linked list from the given data.
 * @param data Data to be inserted in the node.
 * @param weight Weight to be associated with the edge associated with the node.
 * @returns Void
 * */
...
/**
 * @brief A utility function to check if the given data exists in linked list or not.
...
/**
 * @brief A function which converts linked list in a vector with the edge weight.
 ...
 */
        ...
        public:
            int64_t data;                               ///< The key/value of the node
            adjacency_list::LinkedList *adjacencyList;  ///< The adjacency list for storing edges.
        };
...
/**
 * @brief Allocates a new node in heap for given data as a vertex and also assign
 * adjacency list head pointer.
 ...
 * */
        ...
        public:
/**
 * @brief A utility function to find the index of the given data in vertices.
 ...
 * @returns bool Returns true if the edge is added successfully else if the given node not found, will return false.
* */
...
* */
            bool addWeightedEdge(const std::vector<int64_t> &weightedEdge) {
                ...
                if (edgeFromIndex == -1 or edgeToIndex == -1) {
                    /// if any of the vertex (from or to) not found, that means there can not be an
                    /// edge, return false.
                    return false;
                }
 ...
 * @returns bool Returns true if the edge exists between given two nodes else returns false.
* */
...
/**
 * @brief For a given data, will find all the connected vertices to that node.
 * @param data Data for the node for which you want all the connected nodes.
 * @returns std::vector A vector of vertices from the adjacency list of all the connected nodes.
* */
            std::vector<int64_t> getAllConnectedAsVertex(int64_t data) {
                std::vector<int64_t> temp;
                for (Vertex *vertex : vertices) {
                    if (vertex->data == data) {
                        std::vector<int64_t> indexes = vertex->adjacencyList->getAsList();
...
* */
            void printGraph() {
                for (Vertex *vertex : vertices) {
                    std::cout << "Node: " << vertex->data << " is connected to :" << std::endl;
                    std::cout << "--> ";
 ...
 * The assert statement will check connected nodes for a given vertex against the expected output.
 * @returns void
 * */
    static void testCase_1() {
        ...
        std::vector<std::pair<int64_t, int64_t>> edges{
                ...
                std::pair<int64_t, int64_t>(1, 4),
                std::pair<int64_t, int64_t>(1, 5),
 ...
 * */
    static void testCase_2() {
        ...
        std::vector<int64_t> nodes{56, 78, 13, 900, 67, 11, 0, 66};  /// vertex data to be inserted in graph as vertices
 ...
 * */
    static void testCase_3() {
        ...
        std::vector<int64_t> nodes{1, 2, 3, 4, 5, 6, 7, 8};  /// vertex data to be inserted in graph as vertices
        ...
        bool output = graphs.isConnected(3, 4);  /// a bool to check if two nodes are connected or not
        log("Checking assert...");
