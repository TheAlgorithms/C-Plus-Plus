...
private:
    ...
    void updateRange(int i, int l, int r, int tl, int tr, T val) {
        ...
        if (tl <= l && r <= tr) {
            ...
            propagate(i, l, r);  // Propagate immediately to update the current node
        } else {
            int m = mid(l, r);
    ...
    T range_comb(int i, int l, int r, int tl, int tr) {
        propagate(i, l, r);  // Ensure the node is up to date
        if (tl > r || tr < l) return ID;  // Out of range
        if (tl <= l && r <= tr) return t[i];  // Fully in range
        int m = mid(l, r);
...
public:
    ...
    */
   void updateRange(int l, int r, T val) { updateRange(1, 1, size, l, r, val); }
