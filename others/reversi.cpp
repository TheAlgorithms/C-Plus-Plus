...
//
// Input   : if its a new game then all inputs are from the players, but if an
// 			 old game loaded in then we read the current state of the board from
// 			 a binary file
//
// Process : First we read the data and enter it to a char array to visualize
// the process
//			 then we blow the first atom picked randomly while reading the file
// and after that 			 starts the process of handling every blown neutron
// movement
//
// Output  : Every iteration prints the current state of the board and energy
// output
//
//
//---------------------------------------------------------------------------------------
...
Point findBridgeEnd(Game ggame, int nrow, int ncol, int nrowAdder,
                    int ncolAdder);
...
//                                        Main
//                                        ----
// General      : initializes the game and board and presents the starting
...
int main() {
    ...
    // for each cell start a recursion that gets the start color, direction and
    // returns the end point (x,y) of a bridge if there is one and if not return
    // something like (-1,-1) to know the recursion stopped cause it hit an
    // empty cell or the matrix edge
    ...
    // initialize array
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++) carrArrstartGameBoard[i][j] = '?';
    ...
    std::cout << "Type 1 to Start a New Game" << std::endl;
    std::cout << "Type 2 to Load a Game" << std::endl;
    std::cout << "Type 3 to Exit" << std::endl;
    ...
    switch (nchoise) {
        ...
        case 2:
            std::ifstream file;
            std::cout << "enter game file path" << std::endl;
            std::cin >> flocation;
            file.open(flocation, std::ios::in | std::ios::binary);
            if (file.is_open()) {
...
//                                        start Game
//                                        ----------
// General      :  keeps tabs on the game board, checks for bridges and paints
// them
//				  if needed, changes turns and finally shows the winner with its
// score
//
...
void startGame(Game* game) {
    ...
    do {
        ...
        do {
            ...
            std::cout << "enter row and column to move "
                      << game->ccurrPlayerTurn << " player" << std::endl;
            ...
            (game->carrmatGameBoard[row][col] == game->ccurrPlayerTurn ||
             game->carrmatGameBoard[row][col] ==
                 oppositeColor(game->ccurrPlayerTurn)) ||
            !bisInputValid(row, col));  // if cell is occupied or input isn't
                                        // valid keep asking for new input
    ...
    if (!bboardIsntFull(*game)) {
        ...
        if (nwhite > nblack)
            std::cout << "White Won with " << nwhite << " Pieces!!!"
                      << std::endl;
        else if (nwhite == nblack)
            std::cout << "ohhh its a draw" << std::endl;
        else
            std::cout << "Black Won with " << nblack << " Pieces!!!"
                      << std::endl;
    }
}
...
//                                        count Pieces
//                                        ------------
// General      :  counts the pieces for the specified opponent and returns the
// sum
//
//
...
void exitGame(Game* ggame) {
    ...
    if (canswer == 'y') {
        std::cout
            << "pls write the complete file path to save to with name and "
            << ".dat extension and \\ between each directory" << std::endl;
        std::cin >> szpath;
        ...
        try {
            ...
            std::cout << "Bad saving path! exiting" << std::endl;
            exit(0);
        }
...
bool bsaveGame(const char* cppath, Game ggame) {
    std::ofstream file;
    file.open(cppath, std::ios::out | std::ios::binary);
    if (file.is_open()) {
...
bool bboardIsntFull(Game ggame) {
    for (int nrow = 0; nrow < 8; nrow++)
        for (int ncol = 0; ncol < 8;
             ncol++)  // if at least one cell isn't equal to 'W' or 'B'
            if (ggame.carrmatGameBoard[nrow][ncol] != ggame.ccurrPlayerTurn &&
                ggame.carrmatGameBoard[nrow][ncol] !=
                    oppositeColor(ggame.ccurrPlayerTurn))
                return true;
    return false;
}
...
//                                        paint Bridge
//                                        ------------
// General      :  given a game and a bridge it paints the appropriate locations
// the bridge crosses
//
//
...
void paintBridge(Bridge bridge, Game* game) {
    if (bridge.A.nrow == bridge.B.nrow) {                // same row
        ...
        int col =
            (bridge.B.ncol > bridge.A.ncol) ? bridge.A.ncol : bridge.B.ncol;
        ...
        int row =
            (bridge.B.nrow > bridge.A.nrow) ? bridge.A.nrow : bridge.B.nrow;
        for (int i = 0; i < times; i++) {
        ...
        if (bridge.B.ncol > bridge.A.ncol) {
            ...
            if (bridge.B.nrow < bridge.A.nrow) {
                ...
                for (int i = times; i > 1; i--)
                    game->carrmatGameBoard[row - i][col + i] =
                        game->ccurrPlayerTurn;
            }
            // down right
            else {
                int times = abs(bridge.B.nrow - bridge.A.nrow);  // distance
...
//				  Game* game - to access current players turn and current board
// status (In)
//
//
// Return Value : none
    ...
    for (int row = 0; row < 8; row++)
        for (int col = 0; col < 8; col++)
            if (game->carrmatGameBoard[row][col] == colorToSearch) {
                ...
                b[4] =
                    findBridgeEnd(*game, row - 1, col - 1, -1, -1);  // up left
                b[5] =
                    findBridgeEnd(*game, row - 1, col + 1, -1, +1);  // up right
                b[6] = findBridgeEnd(*game, row + 1, col - 1, +1,
                                     -1);  // down left
                b[7] = findBridgeEnd(*game, row + 1, col + 1, +1,
                                     +1);  // down right
                Bridge bridges[8];
                bridges[0] = {a, b[0]};
                bridges[1] = {a, b[1]};
                ...
                bridges[7] = {a, b[7]};
                for (int i = 0; i < 8; i++) {
                    // calculate distance to exit if bridge is too short i.e.
                    // nothing in between
                    int distance = (int)sqrt(
                        pow(bridges[i].A.ncol - bridges[i].B.ncol, 2) +
                        pow(bridges[i].A.nrow - bridges[i].B.nrow, 2));

                    if (distance > 1 &&
                        bisNewBridge((pbridges + i)->A, bridges[i].A,
                                     pbridges) &&
                        bridges[i].B.ncol !=
                            -1) {  // if bridge is new and real then assign it
...
//
//
// Return Value : boolean value of is there a bridge with specified points A and
// B
//
//---------------------------------------------------------------------------------------
...
bool bisNewBridge(Point A, Point B, Bridge* pbridges) {
    for (int i = 0; i < nBridges; i++)
        if (barePointsEqual((pbridges + i)->A, A) &&
                barePointsEqual((pbridges + i)->B, B) ||
            barePointsEqual((pbridges + i)->B, A) &&
                barePointsEqual((pbridges + i)->B, A))
            return false;
    return true;
}
...
// get next location checking spot in requested direction by row adder and col
// adder
Point findBridgeEnd(Game game, int nrow, int ncol, int nrowAdder,
                    int ncolAdder) {
    Point b;
    if (nrow != 0) {
        if (game.carrmatGameBoard[nrow][ncol] == game.ccurrPlayerTurn ||
            ...
            return b;
        } else if (game.carrmatGameBoard[nrow][ncol] ==
                   oppositeColor(game.ccurrPlayerTurn)) {
            return findBridgeEnd(game, nrow + nrowAdder, ncol + ncolAdder,
                                 nrowAdder, ncolAdder);
        }
    }
...
void visualizeBoard(char carrboard[8][8]) {
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) std::cout << carrboard[i][j] << " ";
        std::cout << std::endl;
    }
}
