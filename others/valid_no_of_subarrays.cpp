/**
 * @brief Finds Total No. of Subarrays whose net product is less than a given No. = k
 * @file C-Plus-Plus\others\valid_no_of_subarrays.cpp
 ...
 * @author https://github.com/hecker1002
...
Procedure -
Go in a loop (j=0 to n) and create a sliding window each time starting from the current element and increase its size (in the second loop)
till both of these conditions are true  (product of all elements in the window < k and iterator < last_index)
Proof -
If at any instance, the product of all elements in the current window (subarray) exceeds k, then all further windows' product > k
and hence we shift to the element to the immediate right and do the same process.
*/
 ...
 */
    ...
    int count_SubArray_Product_Less_Than_K(int a[], int n, int k) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            // and with respect to current element of array , initialize the window of size =1.
            long long int product = 1;

            // expanding a window continuously till (product of elements in current window  < k) AND (iterator < last index of array).
            // if any of these above conditions vio,ated , break the loop and count all the valid subarrays till now.
            while (i < n && product < k) {
