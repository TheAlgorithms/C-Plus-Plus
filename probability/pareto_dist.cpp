 ...
 * rule. When modeling phenomena with a heavy tail, including wealth
 distribution,
...
std::vector<double> pareto_distribution(double shape, double scale, int size) {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        double x = scale / std::pow(u, 1.0 / shape);            // Apply the Pareto distribution formula
        data.push_back(x);
    }
...
/**
 * @brief Test function to verify that the generated values ​​are correct
 */
static void tests() {
    double shape = 3.0;                                       
    double scale = 1.0;  
    ...
    // Test 2: Check if all values ​​are greater than or equal to the value minimum (scale)
    for (double value : data) {
        assert(value >= scale);
    }

    // Test 3: Check if the average of the generated values ​​is greater than the parameter scale
    double sum = std::accumulate(data.begin(), data.end(), 0.0);
    double mean = sum / size;
    std::cout << "Average " << mean << std::endl;

    // As the average tends to be greater than the scale parameter (xm) in Pareto distribution
    assert(mean > scale);

    // Test 4: Check if the maximum value is significant in a distribution Pareto (reasonably higher than average)
