...
/**
 *@brief Class defining the structure of trie node and containing the methods to perform operations on them
 */
...
public:
	...
	void SuggestFreqAutocomplete(Tnode* new_root, string prefix, priority_queue<pair<int, string> >& suggestions); 
...
/**
 * Helper Function for Delete : Recursively deletes the substring character by character iterating through the string to be deleted.
 * It traverses till the end of word in a recursive fashion, from there it deletes characters one by one till it reaches back to the initial call.
 ...
 * @param remove_index : Index denoting the beginning of the substring to be deleted
 */
void Tnode :: DeleteFrom(Tnode* delete_from, string delete_string, int remove_index){
...
void Tnode :: Delete(string entry){
	...
	for(i=0; i<n; i++){
		...
		//If the current node is end of word for the current prefix or if it has 2 or more branches
		//It cannot be deleted while deleting the required entry. 
		if (numberOfChildren(cur_pos) > 1 || cur_pos->endOfWord){
			delete_from = cur_pos;	//denotes the beginning of the shortest suffix that is allowed to be deleted
			delete_from_index = i-1;	//Beginning index of the suffix corresponding to the 'entry'
		}
	...
	//cur_pos now points to the last char of entry. Display message if that entry does not exist
	if(!cur_pos->endOfWord){
		cout<<"Entry not Found"<<endl;
		return;
	}

	//If cur_pos is not a leaf node, unmark end of word and assign 0 to it's frequency for deletion 
	...
	//If number of characters in the suffix are more than 1, recursively delete each character starting from cur_pos
...
void Tnode :: Insert(string entry){
	...
	for(auto i : entry){
		...
		//Allocate a node for each character of entry if not present in the trie
		if (cur_pos->english[letter_index] == NULL){
...
bool Tnode :: SearchPresence(string key){
	...
	//Word is only present in the trie if the key is a valid complete entry and not just a prefix.
	if(cur_pos->endOfWord){
		(cur_pos->frequency)++;
		return true;
	}
...
/**
 * SuggestAutocomplete : Recursive function to suggest all the entries of trie which have a given common prefix
 * @param new_root : Pointer pointing to the node corresponding to the last char of prefix
...
void Tnode :: SuggestAutocomplete(Tnode* new_root, string prefix){
	...
	//Iterate through all 26 nodes as we have to print all strings with the given prefix
	for(int i=0;i<ENGLISH_ALPHABET_SIZE;i++){

		if(new_root->english[i] != NULL){
			//Print the sugestion only if it's a valid complete entry and not just a prefix
 ...
 * "abc", "abcde", "abcdefg", "abcddef" and if the search key is "abcdezz", then the longest
 * common prefix is "abcde" and hence search results will be "abcde", "abcdefg".
...
void Tnode :: SearchSuggestions(string key){
	...
	int letter_index = 0;
	string prefix = "";	//variable storing the updated value of longest common prefix

	for(auto i : key){

		letter_index = tolower(i)-97;
		prev_pos = cur_pos;	//Previous pointer updated to point to the last char of the longest common prefix
		
		//When the node for the character does not exist, longest prefix has been determined and SuggestAutocomplete is called
		if (cur_pos->english[letter_index] == NULL){
	...
	//Call for suggestions when the search key is present as an entry/a prefix in the trie
...
/**
 * SelectionTop_3 : Function to display the 3 suggestions with highest frequency of search hits
 * @param suggestions : A max heap that contains pairs of (frequency, word) heapified based on frequency
 */
...
/**
 * SuggestFreqAutocomplete : Recursive function to suggest most frequently searched entries of trie which have a given common prefix
 * @param new_root : Pointer pointing to the node corresponding to the last char of prefix
 * @param prefix : The common prefix that all the suggestions must have
 * @param suggestions : A max heap that contains pairs of (frequency, word) heapified based on frequency
 */
void Tnode :: SuggestFreqAutocomplete(Tnode* new_root, string prefix, priority_queue<pair<int, string> >& suggestions){
	
	for(int i=0;i<ENGLISH_ALPHABET_SIZE;i++){

		if(new_root->english[i] != NULL){
			//Add to sugestions only if it's a valid complete entry and not just a prefix
			if(new_root->english[i]->endOfWord)
				suggestions.push(make_pair(new_root->english[i]->frequency, prefix+char(i+97) ));
...
/**
 * SearchSuggestions : Lists out the most frequent words in trie with the longest prefix
 * of the search key that is present in the trie. For example - if trie contains
 * "abc", "abcde", "abcdefg", "abcddef" and they have been previously searched for 3, 1, 2, 4 times respectively,
 * if the search key is "ab", then the longest common prefix is "ab" and only the top 3 frequencies among the
 * matches would be displayed viz. "abcddef", "abc", "abcdefg". 
 * @param key : The string key to be searched for suggestions
 */
void Tnode :: SearchFreqSuggestions(string key){
	...
	int letter_index = 0;
	string prefix = "";	//variable storing the updated value of longest common prefix
	priority_queue<pair<int, string> >suggestions;	//max heap to store (frequency, word) in descending order of freq
	...
	for(auto i : key){
		...
		//When the node for the character does not exist, longest prefix has been determined and SuggestFreqAutocomplete is called
		if (cur_pos->english[letter_index] == NULL){
...
int main(int argc, char const *argv[])
{
	...
	vector<string> inputs = {"abcde","sss","ssss","ssst","sssu","sssv","sst","ssts","sstt","sstu","tutu","tutuv","tutuu","tutuvs","tutus","tvst","tvsu","vvvv"};
