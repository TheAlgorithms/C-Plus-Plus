...
//Data Structure stores initial array and its nodes, which has the 4th size of the array = 4*N
 struct SegmentTree{
     int N;
     vector<int> arr;
     vector<Node> nodes;
 ...
 // PreProcessing
  void BuildTree_Procedural(int v,int tl,int tr){
      if (tl==tr) ST.nodes[v].val=ST.arr[tl-1],ST.nodes[v].ind=tl;
         else {
            int tm=(tl+tr)/2;
 ...
 Node getMaximumOnInterval_Procedural(int v,int tl,int tr,int l,int r){
      ...
      int tm=(tl+tr)/2;
      Node p,q;
      p=getMaximumOnInterval_Procedural(2*v,tl,tm,l,min(r,tm));
      q=getMaximumOnInterval_Procedural(2*v+1,tm+1,tr,max(l,tm+1),r);
      if (p.val>q.val) return p; else return q;
 };
...
int main(){   
    ...
    //Debug
    vector<int> exampleArray{17,3,190,2,34,-90,19,100,9};
    ...
    for (int i=0;i<exampleArray.size();i++){
        cout<<exampleArray[i];
        if (i!=exampleArray.size()-1) cout<<",";
    }
    cout << "} on interval (2,7) Maximum is " << answer.val << " at index "<<answer.ind<<endl; 
