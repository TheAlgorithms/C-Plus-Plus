...
class Polygon_Area {
 ...
 public:
    ...
    T get_polygon_area() {
        ...
        // first "shoelace" term, it is the sum of the x value of every point
        // not including the last point multiplied by the y value of the point
        // after
        auto first_term =
            std::inner_product(points.begin(), std::prev(points.end(), 1),
                               std::next(points.begin(), 1), 0, std::plus<T>(),
                               multiply_xy_of_two_points);
        // second "shoelace" term, it is the sum of the y value of every point
        // not including the last point multiplied by the x value of the point
        // after
        auto second_term = std::inner_product(
            std::next(points.begin(), 1), points.end(), points.begin(), 0,
            std::plus<T>(), multiply_xy_of_two_points);
        // final area, which is a function of the two terms, the last point's x
        // value multiplied by the first point's y value and the first point's x
        // value multiplied by the last point's y value
