 ...
 *  (A* SEARCH)](https://en.wikipedia.org/wiki/A*_search_algorithm) implementation
 *
 *  \author [Nirmalya Pal](http://github.com/SpEXterXD)
 *
 *  \details
 *  A* Search is an informed search algorithm that is used to find the shortest path
 *  from a start node to a goal node in a graph. It uses a heuristic to improve the efficiency
 ...
 * @param graph - adjacency list of the graph (node ID -> list of (neighbor ID, cost))
 * @param positions - map of node IDs to their (x, y) coordinates for heuristic calculation
...
std::vector<int> aStarSearch(int start, int goal, 
    ...
    std::priority_queue<Node*, std::vector<Node*>, CompareNode> openSet; // Nodes to be evaluated
    std::unordered_map<int, Node*> allNodes; // To keep track of all nodes

    // Initialize the start node
    Node* startNode = new Node(start);
    startNode->hCost = heuristic(positions.at(start).first, positions.at(start).second,
    ...
    while (!openSet.empty()) {
        Node* currentNode = openSet.top(); // Get the node with the lowest fCost
        openSet.pop();
        ...
        for (const auto& neighbor : graph.at(currentNode->id)) {
            ...
            if (allNodes.find(neighborID) == allNodes.end() || tentativeGCost < allNodes[neighborID]->gCost) {
                ...
                neighborNode->hCost = heuristic(positions.at(neighborID).first, positions.at(neighborID).second,
                                                 positions.at(goal).first, positions.at(goal).second);
