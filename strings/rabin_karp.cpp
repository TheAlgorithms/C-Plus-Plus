 ...
 * \param[in] current_char current index of string plus size of pattern
 * \param[in] new_hash previous hash of substring minus previous char of string
 * \param[in] pattern_length length of substring to hash
 * \returns new hash integerprevious
 */
int64_t recalculate_hash(char current_char, int64_t new_hash,
                         size_t pattern_length) {
    new_hash /= PRIME;
    new_hash +=
        (int64_t)(current_char * (int64_t)pow(PRIME, pattern_length - 1));
...
int rabin_karp(const std::string& text, const std::string& pattern) {
    ...
    for (size_t i = 0; i <= string_variance; ++i) {
        ...
        if (i < string_variance) {
            hashed_text = recalculate_hash(text[i + pattern_size],
                                           hashed_text - text[i], pattern_size);
        }
    }
