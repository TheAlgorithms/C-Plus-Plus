 ...
 * operator and size() method \tparam Key the key type, Key must support (== &&
 * <) operators \param container a container to search in it's elements \returns
 * -1 there is no matched key in the container \returns index of the key in the
 * container if found.
 */
...
int binary_search(const Container& container, const Key& key) {
    ...
    while (left_limit <= right_limit) {
        ...
        /// if desired key is grater than the key at the middle index, it should
...
class TestCases {
 ...
 public:
     ...
     *'ContainerClass' container with 'std::string' keys
     */
    void test_case4() {
