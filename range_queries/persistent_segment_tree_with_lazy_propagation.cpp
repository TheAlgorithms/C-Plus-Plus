...
class perSegTree {
 private:
    class Node {
     public:
        ...
        int64_t val, prop;  /// val is the value of the node (here equals to the
    ...
    std::vector<std::shared_ptr<Node>>
        ptrs;  /// ptrs[i] holds a root pointer to the segment tree after the
               /// ith update. ptrs[0] holds a root pointer to the segment tree
               /// before any updates
    std::vector<int> vec;  /// values of the leaf nodes that the segment tree
     ...
     */
    std::shared_ptr<Node> update(const int &i, const int &j, const int &l,
                                 const int &r, const int &value,
     ...
     */
    int64_t query(const int &i, const int &j, const int &l, const int &r,
                  std::shared_ptr<Node> const &curr) {
        lazy(i, j, curr);
        if (j < l || r < i) {
            return 0;
 ...
 public:
     ...
     */
    void construct(
        const std::vector<int> &vec)  // the segment tree will be built from the
                                      // values in "vec", "vec" is 0 indexed
    {
        if (vec.empty()) {
            return;
        }
    ...
    void update(
        ...
        const int &value)  // all elements from index "l" to index "r" would by
                           // updated by "value", "l" and "r" are 0 indexed
    {
     ...
     * @returns sum of elements whose index x satisfies l<=x<=r
     */
    int64_t query(const int &l, const int &r,
                  const int &version)  // querying the range from "l" to "r" in
                                       // a segment tree after "version"
                                       // updates, "l" and "r" are 0 indexed
    {
     ...
     */
    int size()  // returns the number of segment trees (versions) , the number
                // of updates done so far = returned value - 1 ,because one of
                // the trees is the original segment tree
    {
        return ptrs.size();
    }
};
