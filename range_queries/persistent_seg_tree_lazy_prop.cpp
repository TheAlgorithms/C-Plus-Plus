 ...
 */
    ...
    private:
        ...
        public:
            ...
            int64_t val = 0, prop = 0; /// val is the value of the node (here equals to the sum of the leaf nodes children of that node), prop is the value to be propagated/added to all the leaf nodes children of that node
        };
        ...
        std::vector<int64_t> vec{}; /// values of the leaf nodes that the segment tree will be constructed with
         ...
         */
        std::shared_ptr<Node>
        update(const uint32_t &i, const uint32_t &j, const uint32_t &l, const uint32_t &r, const int64_t &value,
    ...
    public:
         ...
         */
        void construct(
                const std::vector<int64_t> &vec) // the segment tree will be built from the values in "vec", "vec" is 0 indexed
        {
            if (vec.empty()) {
                return;
            }
        ...
        void update(const uint32_t &l, const uint32_t &r,
                    const int64_t &value) // all elements from index "l" to index "r" would by updated by "value", "l" and "r" are 0 indexed
        {
        ...
        int64_t query(const uint32_t &l, const uint32_t &r,
                      const uint32_t &version) // querying the range from "l" to "r" in a segment tree after "version" updates, "l" and "r" are 0 indexed
        {
