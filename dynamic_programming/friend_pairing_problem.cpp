 ...
 * this friend decides to go with one of (n-1) friends then the total number of
 * ways will depend upon remaining (n-2) friends. so the final transition is..
 ...
 * @brief Functions for [Friend pairing
 * problem](https://www.geeksforgeeks.org/friends-pairing-problem)
 * Implementation
 */
...
uint64_t calculate_total(const int& n) {
    ...
    std::vector<uint64_t> dp(
        n + 1, 0);      /// dp[i] shows number of ways of pairing i friends.
    dp[0] = dp[1] = 1;  /// trivial base cases, if there are no friends, or only
                        /// one friend then only one way is possible.
