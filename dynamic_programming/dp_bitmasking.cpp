/**
 * @file
 * @brief Functions for [Bitmasking DP
 * approach](https://codeforces.com/blog/entry/337) implementation
 *
 * @details
    ...
    for (uint32_t i = 0; i < M; i++) {
        ...
        for (uint8_t j1 = 0; j1 < total_unselected; j1++) {
            for (uint8_t j2 = j1 + 1; j2 < total_unselected; j2++) {
                uint8_t x = unselected_elements[j1],
                        y = unselected_elements[j2];
                uint32_t new_mask = i | (1 << x) | (1 << y);
                dp[new_mask] =
                    std::max(dp[new_mask], dp[i] + (total_pairs + 1) * G[x][y]);
            }
        }
    }
