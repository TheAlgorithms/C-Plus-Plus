/**
 * @file
 * @brief [Newton's Second
 * Law](https://en.wikipedia.org/wiki/Newton%27s_laws_of_motion) implementation
 * @details Newton's second law states that the force of an object is equal to
 * its mass x its acceleration (F=Ma)
 *
 ...
 * @brief Functions for the [Newton's Second
 * Law](https://en.wikipedia.org/wiki/Newton%27s_laws_of_motion) equation
 */
namespace newtons_second_law {
/**
 * @brief The calculation for [Newton's Second
 * Law](https://en.wikipedia.org/wiki/Newton%27s_laws_of_motion) F=Ma equation
...
static void test() {
    ...
    float float_force_output =
        physics::newtons_second_law::newtons_second_law_of_motion(
            float_mass, float_acceleration);  // force output

    std::cout << "Force of object (float)" << std::endl;
    std::cout << "Input mass (kg): " << float_mass << std::endl;
    std::cout << "Input acceleration (m/s^2): " << float_acceleration
              << std::endl;
    ...
    int int_force_output =
        physics::newtons_second_law::newtons_second_law_of_motion(
            int_mass, int_acceleration);  // force output

    std::cout << "Force of object (int)" << std::endl;
    std::cout << "Input mass (kg): " << int_mass << std::endl;
    std::cout << "Input acceleration (m/s^2): " << int_acceleration
              << std::endl;
